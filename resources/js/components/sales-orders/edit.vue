    ...
    <template v-for="(selected_item, index) in form.items" :key="index">
      ...
      <div class="col-md-1">
        ...
        <div class="btn-group btn-group-sm">
          <button type="button" class="btn btn-danger" @click="removeRow(index)" :disabled="form.items.length === 1">
...
export default {
  ...
  data() {
    return {
      form: useForm('put', `/sales-orders/${this.sales_order.id}`, {
        items: this.sales_order.items.map(item => ({
          id: item.id,
